...
Для расчета маркетов SettlementEngine опирается на журнал события. Заполнение журнала события происходит с помощью методов *APPEND-EVENT-LOG* и *SET-EVENT-LOG* эти методы возвращают маркеты рассчитанные на основе нового журнала события.
...
* filters - опционально, hashmap, название столбцов и список значений столбцов которые должны остаться при получении расчета
